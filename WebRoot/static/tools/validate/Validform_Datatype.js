			...
			*/
			...
			"numrange":function(gets,obj,curform,datatype){
				...
				*/
				...
				var maxim=Number(obj.attr("max")) || 100000000.00,
					minim=Number(obj.attr("min")) || 0.00;
				...
				if(gets<minim){
					return itemName+"不能小于"+minim+"！";
				}else if(gets>maxim){
					return itemName+"不能大于"+maxim+"！";
				}
				return  true;
			},
            ...
            //经纬度
			"lnglat":/^[-]?(\d|([1-9]\d)|(1[0-7]\d)|(180))(\.\d*),[-]?(\d|([1-8]\d)|(90))(\.\d*)?$/,
            //空值
            "empty":/^\s*$/,
            "onemore":function(gets,obj,curform,datatype){
				...
				var reg1=/[\w\W]+/,
					productLicense=curform.find("input[name='productLicense']");
                    otherLicense=curform.find("input[name='otherLicense']");
                    smallWorkShopLicense=curform.find("input[name='smallWorkShopLicense']");
                    foodCirculationLicense=curform.find("input[name='foodCirculationLicense']");
                    
				
				if(reg1.test(productLicense.val())){return true;}
                if(reg1.test(otherLicense.val())){return true;}
                if(reg1.test(smallWorkShopLicense.val())){return true;}
                if(reg1.test(foodCirculationLicense.val())){return true;}
