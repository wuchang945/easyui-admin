...
public class ZXingCode {
    ...
    {  
        try  
        {  
            String s=getLogoQRCode("https://www.baidu.com/", null, "跳转到百度的二维码",null);
    ...
    public static String getLogoQRCode(String qrUrl,HttpServletRequest request,String productName,String ddbh)
    {
    	String filePath = request.getSession().getServletContext().getRealPath("/") + "static/images/index/logo.png";
        //filePath是二维码logo的路径，但是实际中我们是放在项目的某个路径下面的，所以路径用上面的，把下面的注释就好
        //String filePath = "D://111.jpg";  //TODO 
        ...
        try
        {
            ...
            return zp.addLogo_QRCode(bim, new File(filePath), new LogoConfig(), productName,request,ddbh);
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    public String addLogo_QRCode(BufferedImage bim, File logoPic, LogoConfig logoConfig, String productName,HttpServletRequest request,String ddbh)
    {
        try
        {
            /** * 读取二维码图片，并构建绘图对象 */
...
// outg.drawString(productName, 0, image.getHeight() + (outImage.getHeight() - image.getHeight())/2 + 5 ); //画文字
            ...
            String datename=DateUtil.date2String(new Date(), "yyyyMMdd");
            File file=new File(request.getSession().getServletContext().getRealPath("/")+"static/qrcode/" +datename );
            if(!file.exists()){
            	file.mkdirs();
            }
            ImageIO.write(image, "png", new File(request.getSession().getServletContext().getRealPath("/")+"static/qrcode/" + datename+ "/" +ddbh+".png")); //TODO 
            String imageBase64QRCode =  MyBase64.byteArrayToBase64(baos.toByteArray());
            baos.close();
